...
<div class="page-content clearfix">
    ...
    <div class="search_style">
      ...
      <ul class="search_content clearfix">
       <li><label class="l_f">会员名称</label><input name="userName" id="userName" type="text" value="" class="text_add" placeholder="输入会员名称" style=" width:400px"/></li>
       <li><label class="l_f">添加时间</label><input class="inline laydate-icon" id="start" style=" margin-left:10px;"></li>
       <li style="width:90px;"><button type="button" class="btn_search" onclick="selLocal();"><i class="icon-search"></i>查询</button></li>
      </ul>
    </div>
     <!---->
     <div class="border clearfix">
       ...
       <span class="r_f">共：<b id="allCount">2345</b>条</span>
     </div>
     <!---->
...
<script>
    ...
    function init() {
        ...
        $("#hi").html("");
        $.ajax({
            url:'${pageContext.request.contextPath}/selAllUser.do',
            type:'post',
            data:{
                "uName":$("#userName").val(),
                "uTime":$("#start").val(),
            ...
            success:function(data){
                ...
                $.each(obj,function(i,x){
                    ...
                    if (x.flag==1) {
                        state="已启用";
                        tds="success";
                        btn="<a class='btn btn-xs' onClick='member_stop(this,"+x.userId+")' href='javascript:;' title='停用'>停用</a>";
                    } else if(x.flag==3){
                        state="已停用";
                        tds="defaunt";
                        btn="<a class='btn btn-xs btn-success' onClick='member_start(this,"+x.userId+")' href='javascript:;' title='启用'><i class=\"icon-ok bigger-120\"></i></a>";
                    }

                    // http://localhost:8080/bom/img/)(RO}BLH~JI60Z{OVFA`9_I.png
                    $str=$str+"<tr><td><label><input type='checkbox' name='Multiple_Selection' class='ace' value='"+x.userId+"'><span class='lbl'></span></label></td>"
                        +"<td>"+x.userId+"</td>"
                        +"<td>"+x.name+"</td>"
                        +"<td>"+sex+"</td>"
                        +"<td>"+x.phone+"</td>"
                        +"<td>"+x.age+"</td>"
                        +"<td><img src='"+x.headImg+"' width='90px' height='60px'></td>"
                        +"<td>"+x.uTime+"</td>"
                        +"<td>"+power+"</td>"
                        +"<td class='td-status'><span class='label label-"+tds+" radius'>"+state+"</span></td>"
                        +"<td class='td-manage'>"+btn+" " +
                       // "<a title='编辑' onclick="+"member_edit("+x.userId+",'member-add.html','4','','510')"+" href='javascript:;' class='btn btn-xs btn-info'><i class=\"icon-edit bigger-120\"></i></a> " +
                        "<a title='删除' href='javascript:;' onclick="+"member_del(this,"+x.userId+")"+" class=\"btn btn-xs btn-warning\"><i class=\"icon-trash  bigger-120\"></i></a></td>"
                        +"</tr>";
...
function member_stop(obj,id){
	layer.confirm('确认要停用吗？',function(index){
        ...
        // layer.msg('已停用!',{icon: 5,time:1000});
            ...
            // $(obj).remove();
            // layer.msg('已停用!', {icon: 5, time: 1000});
            $.post("${pageContext.request.contextPath}/upUserStop.do",{userId:id},function(data){
                if (data==1) {
                    layer.msg('已停用!',{icon: 5,time:1000});
                } else if(data==0) {
...
function member_start(obj,id){
	layer.confirm('确认要启用吗？',function(index){
        ...
        $.post("${pageContext.request.contextPath}/upUserStart.do",{userId:id},function(data){
            if (data==1) {
                layer.msg('已启用!',{icon: 6,time:1000});
            } else if(data==0) {
...
function member_del(obj,id){
	layer.confirm('确认要删除吗？',function(index){
        ...
        $.post("${pageContext.request.contextPath}/upUserState.do",{userId:id},function(data){
            if (data==1) {
                layer.msg('已删除!',{icon:1,time:1000});
            } else if(data==0) {
    ...
    function multDel() {
        ...
        layer.confirm('确认要删除这些商品吗？',function(index){
           ...
           // alert(MDuserState);
            ...
            $.post("${pageContext.request.contextPath}/upUserMaryDel.do",{userIds:MDuserState},function(data){
